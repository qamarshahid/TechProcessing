...
interface AddCloserModalProps {
  onClose: () => void;
  onSuccess: () => void;
}

export function AddCloserModal({ onClose, onSuccess }: AddCloserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.createCloser(formData);
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error('Error creating closer:', err);
      setError(err.message || 'Failed to create closer');
    } finally {
      setLoading(false);
    }
  };
