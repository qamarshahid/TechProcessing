...
# Create Firebase secrets if they don't exist
echo "🔐 Creating Firebase secrets..."
echo "Please enter your Firebase Server Key (starts with 'AAAA'):"
read -s FIREBASE_SERVER_KEY
echo "$FIREBASE_SERVER_KEY" | gcloud secrets create firebase-server-key --data-file=- --quiet 2>/dev/null || echo "Secret already exists, updating..."
echo "$FIREBASE_SERVER_KEY" | gcloud secrets versions add firebase-server-key --data-file=-
