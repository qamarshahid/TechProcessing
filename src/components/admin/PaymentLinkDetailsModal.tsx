...
export function PaymentLinkDetailsModal({ link, onClose, onLinkUpdated }: PaymentLinkDetailsModalProps) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'ACTIVE':
        return <CheckCircle className="h-5 w-5 text-accent500" />;
      case 'USED':
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'ACTIVE':
        return 'bg-accent100 text-accent800 border-accent200';
      case 'USED':
      ...
      <div className="bg-white dark:bg-surface rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-outline">
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-fg">{link.title}</h2>
              <p className="text-sm text-gray-600 dark:text-muted">Payment Link Details</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-surface2 rounded-lg">
                <div className="flex items-center">
                  <User className="h-5 w-5 text-muted mr-3" />
                  <span className="text-sm text-gray-600 dark:text-muted">Client</span>
                </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-surface2 rounded-lg">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-gray-600 dark:text-muted">Amount</span>
                </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-surface2 rounded-lg">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-gray-600 dark:text-muted">Created</span>
                </div>
                <span className="font-medium text-gray-900 dark:text-fg">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-surface2 rounded-lg">
                <div className="flex items-center">
                  <Calendar className="h-5 w-5 text-muted mr-3" />
                  <span className="text-sm text-gray-600 dark:text-muted">Expires</span>
                </div>
                <span className={`font-medium ${isExpired ? 'text-red-600' : 'text-gray-900 dark:text-fg'}`}>
              ...
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    value={paymentUrl}
                    readOnly
                    className="flex-1 px-3 py-2 bg-gray-50 dark:bg-surface2 border border-gray-300 dark:border-slate-600 rounded-lg text-sm font-mono text-gray-900 dark:text-fg"
                  />
                  <button
                    onClick={() => copyToClipboard(paymentUrl, 'Payment URL')}
                    className="p-2 bg-blue-600 text-fg rounded-lg hover:bg-blue-700 transition-colors"
                    title="Copy URL"
                  >
              ...
              <div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="p-2 bg-gray-600 text-fg rounded-lg hover:bg-gray-700 transition-colors"
                    title="Copy Token"
                  >
                    {copiedText === 'Token' ? (
                      <CheckCircle className="h-4 w-4" />
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      readOnly
                      className="flex-1 px-3 py-2 bg-gray-50 dark:bg-surface2 border border-gray-300 dark:border-slate-600 rounded-lg text-sm text-gray-900 dark:text-fg"
                    />
                    <button
                      onClick={() => copyToClipboard(link.client.email, 'Email')}
                      className="p-2 bg-accent600 text-fg rounded-lg hover:bg-accent700 transition-colors"
                      title="Copy Email"
                    >
                      {copiedText === 'Email' ? (
                        <CheckCircle className="h-4 w-4" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <button
              ...
              className="flex items-center justify-center px-4 py-3 bg-accent600 text-fg rounded-lg hover:bg-accent700 transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center justify-center px-4 py-3 bg-purple-600 text-fg rounded-lg hover:bg-purple-700 transition-colors"
            >
            ...
            <a
              ...
              className="flex items-center justify-center px-4 py-3 bg-blue-600 text-fg rounded-lg hover:bg-blue-700 transition-colors"
            >
          ...
          <div className="bg-gray-50 dark:bg-surface2 rounded-lg p-4">
            <h3 className="text-sm font-medium text-gray-900 dark:text-fg mb-3">Link Statistics</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-600 dark:text-muted">Created:</span>
                <span className="ml-2 text-gray-900 dark:text-fg">
                  {new Date(link.created_at).toLocaleString()}
                </span>
              </div>
              <div>
                <span className="text-gray-600 dark:text-muted">Last Updated:</span>
                <span className="ml-2 text-gray-900 dark:text-fg">
              ...
              {link.usedAt && (
                <div className="col-span-2">
                  <span className="text-gray-600 dark:text-muted">Payment Completed:</span>
                  <span className="ml-2 text-gray-900 dark:text-fg">
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-gray-200 dark:border-outline flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gray-100 dark:bg-surface2 text-gray-700 dark:text-muted rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors"
          >
            Close
          </button>
        </div>
