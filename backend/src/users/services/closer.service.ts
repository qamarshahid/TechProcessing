...
export class CloserService {
  ...
  async getCloserStatsForManagement(closerId: string): Promise<any> {
    ...
    // Count all sales (including pending) but exclude rejected and revoked sales
    const totalSales = await this.agentSaleRepository.count({
      where: { 
        closerId,
        saleStatus: Not(In([SaleStatus.REJECTED, SaleStatus.REVOKED]))
      },
    });
    ...
    const totalSalesValue = await this.agentSaleRepository
      ...
      .select('SUM(sale.saleAmount)', 'total')
      .where('sale.closerId = :closerId', { closerId })
      .andWhere('sale.saleStatus NOT IN (:...excludedStatuses)', { excludedStatuses: [SaleStatus.REJECTED, SaleStatus.REVOKED] })
      .getRawOne();
