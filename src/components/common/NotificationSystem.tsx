...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'success': return <CheckCircle className="h-5 w-5 text-accent500" />;
  ...
  const getColors = (type: string) => {
    switch (type) {
      case 'success': return 'bg-accent50 dark:bg-accent900/20 border-accent200 dark:border-accent800';
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                {notification.message && (
                  <p className="text-sm text-gray-700 dark:text-muted mt-1">
                    {notification.message}
                  </p>
                )}
              </div>
              <button
                ...
                className="ml-3 text-muted hover:text-gray-600 dark:text-gray-500 dark:hover:text-muted transition-colors"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
