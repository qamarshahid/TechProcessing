    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">Manage recurring monthly services</p>
        </div>
        <button 
          ...
          <div key={subscription.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <h3 className="text-lg font-semibold text-gray-900">{getServiceName(subscription)}</h3>
                  <div className="flex items-center text-green-600">
                    <DollarSign className="h-4 w-4" />
                    <span className="font-bold">{parseFloat(subscription.amount || '0').toLocaleString()}</span>
                    <span className="text-sm text-gray-500 ml-1">/{getFrequencyText(subscription.frequency).toLowerCase()}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">{formatDate(subscription.nextBillingDate || subscription.next_billing_date)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Started:</span>
                <span className="text-gray-900">{formatDate(subscription.startDate || subscription.start_date || subscription.createdAt || subscription.created_at)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Total Billed:</span>
                <span className="text-gray-900">${parseFloat(subscription.totalBilled || subscription.total_billed || '0').toLocaleString()}</span>
              </div>
            </div>
      ...
      {subscriptions.length === 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 mb-6">Create your first subscription to get started with recurring billing.</p>
          <button 
            onClick={() => setShowCreateModal(true)}
            className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{getClientName(selectedSubscription)}</p>
                </div>
              </div>
              <button
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">{getClientName(selectedSubscription)}</p>
                  </div>
                </div>
              </div>
